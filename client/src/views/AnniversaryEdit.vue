...
onMounted(async () => {
  if (!editingId) return
  try {
    const a = await api.get(`/api/anniversaries/${editingId}`)
...
async function save() {
  ...
  try {
    ...
    if (editingId) await api.put(`/api/anniversaries/${editingId}`, payload)
    else await api.post('/api/anniversaries', payload)
