...
onMounted(async () => {
  if (!editingId) return
  try {
    const w = await api.get(`/api/wishes/${editingId}`)
...
async function save() {
  ...
  try {
    ...
    if (editingId) await api.put(`/api/wishes/${editingId}`, payload)
    else await api.post('/api/wishes', payload)
...
<template>
  <div class="fade-up">
    <div class="mb-4 flex items-center justify-between">
      ...
      <span class="text-xs text-white/40">{{ editingId ? '修改这个心愿' : '想一起做的事' }}</span>
    </div>
    ...
    <div class="flex items-center justify-end gap-3">
      ...
      <button class="btn-primary" :disabled="busy || !form.title.trim()" @click="save">
