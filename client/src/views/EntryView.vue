...
async function load() {
  entry.value = await api.get(`/api/entries/${route.params.id}`)
}
onMounted(load)
...
async function remove() {
  ...
  if (!ok) return
  await api.delete(`/api/entries/${entry.value.id}`)
  toast('日记已删除')
  goBack()
}
...
async function togglePublic() {
  ...
  if (!ok) return
  entry.value = await api.patch(`/api/entries/${entry.value.id}/visibility`, { isPublic })
