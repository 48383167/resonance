...
async function load() {
  try {
    wish.value = await api.get(`/api/wishes/${route.params.id}`)
  } catch (e) {
    toast(e.message)
    router.push('/wishes')
  }
...
async function move(dir) {
  ...
  if (i < 0 || i > 2) return
  try {
    await api.put(`/api/wishes/${wish.value.id}/status`, { status: order[i] })
    wish.value = await api.get(`/api/wishes/${wish.value.id}`)
...
async function remove() {
  ...
  if (!ok) return
  await api.delete(`/api/wishes/${wish.value.id}`)
  toast('已删除')
  router.push('/wishes')
}
