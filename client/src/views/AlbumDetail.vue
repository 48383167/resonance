...
async function loadAlbum() {
  album.value = await api.get(`/api/albums/${route.params.id}`)
}
...
async function loadPhotos(reset = true) {
  ...
  try {
    ...
    const data = await api.get(`/api/albums/${route.params.id}/photos?offset=${offset}&limit=${PAGE}`)
...
async function addUploaded() {
  ...
  for (const url of uploadUrls.value) {
    await api.post(`/api/albums/${album.value.id}/photos`, { url })
  }
  uploadUrls.value = []
...
async function removePhoto(p) {
  ...
  if (!ok) return
  await api.delete(`/api/albums/${album.value.id}/photos/${p.id}`)
...
async function setCover(p) {
  album.value = await api.put(`/api/albums/${album.value.id}/cover`, { url: p.url })
  toast('封面已更新')
}
...
async function saveStory() {
  ...
  if (!p) return
  const updated = await api.put(`/api/albums/${album.value.id}/photos/${p.id}`, { caption: storyText.value })
...
async function removeAlbum() {
  ...
  if (!ok) return
  await api.delete(`/api/albums/${album.value.id}`)
  toast('相册已删除')
  router.push('/albums')
}
