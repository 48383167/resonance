...
import { getCapsule, removeCapsule } from '../capsule.api.js'
...
async function load() {
  try {
    capsule.value = await getCapsule(route.params.id)
  } catch (e) {
    toast(e.message)
...
async function remove() {
  ...
  if (!ok) return
  await removeCapsule(capsule.value.id)
  toast('胶囊已销毁')
  router.push('/capsules')
}
  ...
  <div class="fade-up">
    ...
    <div v-else class="glass p-6 sm:p-10">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <span class="break-words text-xs text-white/40">由 {{ capsule.author?.nickname }} 密封 · 解锁于 {{ capsule.unlock_date }}</span>
      </div>

      <h1 class="serif mt-6 break-words text-2xl font-bold">{{ capsule.title || '无题胶囊' }}</h1>

      <p class="serif mt-6 break-words whitespace-pre-wrap text-base leading-loose"
