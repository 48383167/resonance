...
console.log('== 16. 底部导航（统一设置） ==')
const navInit = await http('GET', '/api/navigation', null, tokenA)
assert('默认底部导航', navInit.ok && navInit.data.items.length === 4 && navInit.data.items[0] === 'home',
  JSON.stringify(navInit.data))
const navBadName = await http('PUT', '/api/navigation', { items: ['home', 'nope'] }, tokenA)
assert('未知入口被拒绝', navBadName.ok === false && navBadName.error?.code === 'INVALID_NAV_ITEMS')
const navDup = await http('PUT', '/api/navigation', { items: ['home', 'home'] }, tokenA)
assert('重复入口被拒绝', navDup.ok === false && navDup.error?.code === 'INVALID_NAV_ITEMS')
const navMany = await http('PUT', '/api/navigation', { items: ['home', 'timeline', 'diary-list', 'moments', 'letters', 'albums'] }, tokenA)
assert('超过 5 个被拒绝', navMany.ok === false && navMany.error?.code === 'INVALID_NAV_ITEMS')
const navEmpty = await http('PUT', '/api/navigation', { items: [] }, tokenA)
assert('空导航被拒绝', navEmpty.ok === false && navEmpty.error?.code === 'INVALID_NAV_ITEMS')
const navSave = await http('PUT', '/api/navigation', { items: ['diary-list', 'home', 'wishes'] }, tokenA)
assert('保存底部导航', navSave.ok && navSave.data.items[0] === 'diary-list' && navSave.data.items.length === 3)
